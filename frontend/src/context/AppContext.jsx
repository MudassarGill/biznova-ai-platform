...
export function AppProvider({ children }) {
  ...
  // ─── AI: Generate Business Ideas (Backend → Gemini AI) ───
  const generateIdeas = useCallback(async (profile) => {
    ...
    try {
      ...
      setBusinessIdeas(Array.isArray(ideas) ? ideas : [])
      showToast(`🚀 ${ideas.length || 6} business ideas generated by AI!`)
    } catch (err) {
      console.warn('Backend AI unavailable, using mock data:', err.message)
      // Fallback to mock data when backend/Gemini is unreachable
      await new Promise(resolve => setTimeout(resolve, 1500))
      setBusinessIdeas(mockBusinessIdeas)
      showToast('🚀 6 business ideas generated! (demo mode)', 'info')
    }
  ...
  const selectIdea = useCallback(async (idea) => {
    ...
    const location = userProfile?.location || 'Islamabad'

    // Load market analysis
    try {
      const marketRes = await fetch(`${API_BASE}/api/analysis/market`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
    ...
    // Load forecast
    try {
      const forecastRes = await fetch(`${API_BASE}/api/analysis/forecast`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          idea_title: idea.title,
    ...
    // Load business plan
    try {
      const planRes = await fetch(`${API_BASE}/api/analysis/plan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          idea_title: idea.title,
      ...
      if (planRes.ok) {
        ...
        setBusinessPlan(planJson)
      } else {
        throw new Error('Business plan failed')
      }
    } catch {
      console.warn('Using mock business plan')
